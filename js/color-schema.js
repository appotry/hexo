...
(function(window, document) {
  ...
  var colorSchemaStorageKey = 'Matery_Color_Scheme';
  ...
  function resetSchemaAttributeAndLS() {
    ...
    setLS(colorSchemaInitStorageKey, getDefaultColorSchema());
  }
  ...
  function applyCustomColorSchemaSettings(schema) {
    ...
    if (current === getDefaultColorSchema()) {
      ...
      setLS(colorSchemaInitStorageKey, current);
  ...
  function setButtonIcon(schema) {
    if (validColorSchemaKeys[schema]) {
      ...
      if (iconElement) {
        ...
        Matery.utils.waitElementLoaded(colorToggleIconSelector, function() {
          var iconElement = document.querySelector(colorToggleIconSelector);
          if (iconElement) {
            iconElement.setAttribute(
              'class',
  ...
  function setHighlightCSS(schema) {
    ...
    var prismlightCss = document.getElementById('prism-css');
    var prismdarkCss = document.getElementById('prism-css-dark');
    if (schema === 'dark') {
      if (darkCss) {
      ...
      if (prismdarkCss) {
        prismdarkCss.removeAttribute('disabled');
      }
      if (prismlightCss) {
        prismlightCss.setAttribute('disabled', '');
      }
    } else {
      if (lightCss) {
      ...
      if (prismlightCss) {
        prismlightCss.removeAttribute('disabled');
      }
      if (prismdarkCss) {
        prismdarkCss.setAttribute('disabled', '');
      }
    }
    ...
    setTimeout(function() {
      ...
      document.querySelectorAll('.markdown-body pre').forEach((pre) => {
        var cls = Matery.utils.getBackgroundLightness(pre) >= 0 ? 'code-widget-light' : 'code-widget-dark';
